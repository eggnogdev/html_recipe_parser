...
/// Parse a recipe out of an HTML file from a given url
class HTMLRecipeParser {
  /// Parse a recipe from the given url
  ///
  /// expecting a url to an HTML page with a schema.org Recipe jsonld
  static Future<Map<String, dynamic>?> parseUrl(String url) async {
    final requestUri = Uri.parse(url);
    final dom = await _getDocument(requestUri);

    return _getJson(dom);
  }

  /// get the DOM from the given uri
  static Future<Document> _getDocument(Uri uri) async {
  ...
  /// get the (first) Recipe JSON in the given DOM
  static Map<String, dynamic>? _getJson(Document dom) {
